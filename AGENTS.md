...
- `test/cli.js` drives the real CLI against stubbed `python`, `ffmpeg`, and `ffplay` placed on `PATH`: no network, no model, no audio device. Shared fixtures are `stubs()`, `stubEnv()`, `cliArgs()`, `stubEvents()`, `waitFor()`, and `isAlive()`. `KKR_STUB_FFPLAY_HOLD=1` keeps a stub player alive so a paused paragraph is observable; `0` lets it exit with its pipe.
- Pause mutes by killing the paragraph's `ffmpeg`/`ffplay` pair, and `resume` replays the same paragraph from an offset `PAUSE_REWIND_MS` puts back. Freezing a player is not an option: `ffplay` stutters out the ~0.25 s of samples already handed to SDL and CoreAudio when it is `SIGSTOP`ped, and after `SIGCONT` it burns the rest of its input at ~6x realtime (ffmpeg 9, macOS) and exits, dropping the remainder. Nothing may suspend a child, because a stopped child also defers `SIGTERM` forever on Linux.
- Only macOS is verified on hardware (mute in 8-9 ms, ~0.25 s of audio already committed to the device at the cut). `PAUSE_REWIND_MS` must stay above whatever the audio server owns at the cut, or Resume skips instead of repeating, so measure Linux (PulseAudio and PipeWire target more latency) before claiming parity. Windows is unmeasurable here: Node terminates the child abruptly, and the deleted PowerShell `Suspend-Process` cmdlets never existed, which is why Pause muted nothing there before 0.1.3.
